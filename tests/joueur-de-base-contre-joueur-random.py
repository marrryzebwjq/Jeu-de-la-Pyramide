...
from Joueur import JoueurRandom, JoueurPresqueRandom
...
# idée :
# faire un joueur qui ne s'améliore pas mais qui est déjà un peut meilleur qu'un random
# ce sera "JoueurPresqueRandom", un joueur un peu meilleur que JoueurRandom
# -> servira à évaluer notre IA
...
j2rand = JoueurRandom("Rachelle")
bot2 = JoueurPresqueRandom("Bernadette")

j1_wins, j2_wins, n_parties, j1, j2 = stats(j1rand, j2rand, nb_val=int(2+5*(5+1)/8)+1, taille_pyramide=5, nb_cartes_par_joueur=4)

print("Partie courte: random vs random")
print(f'j1_wins : {j1_wins/(j1_wins+j2_wins):.2f}%')
print(f'j2_wins : {j2_wins/(j1_wins+j2_wins):.2f}%')

j1_wins, j2_wins, n_parties, j1, j2 = stats(j1rand, bot2, nb_val=int(2+5*(5+1)/8)+1, taille_pyramide=5, nb_cartes_par_joueur=4)

print("Partie courte : random vs probas(bluff=0.2, denonce=0.2)")
print(f'j1_wins : {j1_wins/(j1_wins+j2_wins):.2f}%')
print(f'j2_wins : {j2_wins/(j1_wins+j2_wins):.2f}%')
...
for i in range(len(x)):
    for j in range(len(y)):
        ...
        bot2 = JoueurPresqueRandom("Bernadette", probaBluff=p1, probaDenonce=p2)
        j1_wins, j2_wins, n_parties, j1, j2 = stats(j1rand, bot2, n_parties=500, nb_val=nb_val, taille_pyramide=taille_pyramide, nb_cartes_par_joueur=nb_cartes_par_joueur)
